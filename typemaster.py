...
# the screen that allows user to quit and access all the subpages
    def main_menu(self):
        ...
        # titles
        main_menu_title = self.get_font(self.title_size).render("type master", True, self.title_color)
...
#   iii) return  to the homepage
    def play_menu(self):
        ...
        play_menu_title = self.get_font(self.subtitle_size).render("take a test", True, self.title_color)
        play_menu_title_rect = play_menu_title.get_rect(center=self.title_center)
        # library text subtitle
        library_title = self.get_font(self.label_size).render("library text", True, self.title_color)
        library_title_rect = library_title.get_rect(center=(640, 340))
        # custom text subtitle
        custom_title = self.get_font(self.label_size).render("single test custom", True, self.title_color)
        ...
        custom_text = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect((465, 480), (340, 38)),
            manager=play_menu_manager)
        # main loop
...
# takes a string parameter that will be used as the passage in the test
    def play(self, text):
        ...
        # titles
        play_title = self.get_font(self.title_size).render("test", True, self.title_color)
        ...
        type_progress = pygame_gui.elements.UIStatusBar(
            relative_rect=pygame.Rect((150, 505), (980, 50)),
            manager=play_manager)
        ...
        while play_running:
            ...
            text_display.set_text(f"<font color={correct_c}>{str(correct_typed_chars)}<font color={incorrect_c}>{str(incorrect_typed_chars)}<font color={inactive_c}>{str(remaining_chars)}")
            # get events
            for event in pygame.event.get():
                # window closed
                if event.type == pygame.QUIT:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if time_count:
                        ...
                        if event.unicode in self.allowed_characters:
                            ...
                            if typed_text.peek() == remaining_chars.peek() and incorrect_typed_chars.is_empty():
                        ...
                        if event.key == pygame.K_BACKSPACE and not incorrect_typed_chars.is_empty():
                            # remove last typo chr (as stored in stack)
                            incorrect_typed_chars.pop()
                        text_input.set_text(f"{correct_typed_chars}{incorrect_typed_chars}")
                ...
                if remaining_chars.is_empty():
                    ...
                    accuracy = round((total_char_count / (total_char_count + typo_count)) * 100, 2)
...
# calculates new session statistics
    def results(self, text, time, percent_acc, wpm):
        ...
        # titles
        results_title = self.get_font(self.subtitle_size).render("test results", True, self.title_color)
...
#   IV)   average accuracy per time       
    def stats(self):
        ...
        # titles
        stats_title = self.get_font(self.subtitle_size).render("session stats", True, self.title_color)
...
# or create a new entry in the database
    def library(self):
        ...
        # titles
        manage_library_title = self.get_font(self.subtitle_size).render("manage library", True, self.title_color)
        manage_library_title_rect = manage_library_title.get_rect(center=self.title_center)
        # refresh dictionary
        self.load_keydict()
        # buttons
        back_button = pygame_gui.elements.UIButton(
        ...
        text_entry = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect((300, 460), (680, 38)),
            manager=library_manager)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame_gui.UI_BUTTON_PRESSED:
                    ...
                    if event.ui_element == new_button:
                        ...
                        text_entry.set_text("")
                    if event.ui_element == save_button and len(text_entry.get_text()) != 0:
                        if not new_text:
